# Analysis panels (JavaScript)

**[中文](zh/analysis-panel.md)** · [Docs](index.md)

An analysis panel is a workspace tab whose contents are a JavaScript application you
wrote. JavaScript is the declaration: the panel's `render` function is the description of
the interface, and the range controls, chips, KPI cards, charts and tables are all yours
to compose. A panel reads the database the app already has open — the same tables, views
and attached files — through the host functions below.

This is for building a view over your data: a dashboard, a purpose-built browser for one
table, a report you run every morning. A panel opens and closes like a query, and sits
beside your SQL tabs.

## Open a panel

Any of these opens one, and each loads the folder immediately:

- the **+** at the end of the tab strip, then **Open panel…**;
- drag a panel folder onto the window;
- name it on the command line: `ducklocal examples/analysis_app`.

A folder that cannot be a panel — a file, a path that is gone, or a folder with no
`main.js` — is refused **inside the tab**, naming the folder and what is missing. A panel
that is already working is never thrown away by a folder you cancelled out of or named by
mistake.

Panels close with the tab's close button and rename like a query tab. The panel folders you
...
Panels run on their own DuckDB connection to the database the main window has open, so a
panel and the SQL editor see the same tables, views and attached files without waiting on
...
`SET` made in the SQL editor is not there for a panel, and the other way round. All panels
share that one connection, so they still queue behind each other.

## View definition

**View definition** in the panel's header shows the folder and the entry file's source,
read-only. A panel is an ordinary directory of JavaScript files, so that is the whole
definition — there is no second, hidden form of the panel to inspect.

## Reload

Save a `.js` or `.mjs` file in the panel's folder and the panel reloads. The host polls the
...
**Reload** in the panel's header does the same thing on demand.

A reload that fails **keeps the last panel that loaded** and reports the error above it,
...
They all run against the panel connection — the same database the main window is on — and
they throw JavaScript `Error`s on failure. Everything but `panelDir` may be called at any
time; `panelDir` answers during the panel's load.
...
```ts
panelDir(): string   // the panel's own folder, absolute
```

A panel cannot read the filesystem, so this is how it addresses data that ships beside it —
`query("SELECT * FROM " + sqlLiteral(panelDir() + "/orders.csv"))` works with nothing
attached and nothing open in the app. It answers while the panel loads: call it in `init()`
and keep the result.
...
your panel's own state — a filter, a channel id, a date — instead of templating it into SQL
...
the names a panel builds SQL from are rarely its own — `catalog()` answers with whatever the
...
A panel draws with the same component catalog the shell ships, imported from
...
Every panel folder ships a `jsconfig.json` and a generated `gpui-kit.d.ts`, so check the
panel against the API the runtime will actually give it before you save:

```sh
npx --yes -p typescript tsc -p <panel-dir>/jsconfig.json --noImplicitAny false
```

That reports a property that does not exist on a catalog component — the kind of mistake
the runtime otherwise refuses only at render, inside the panel's own error surface, which
...
`query()` runs on the app's own database, **with the app's privileges and the user's**.
A panel can read and write anything the SQL editor can, including `COPY` to files, `ATTACH`
of other databases, `INSTALL`/`LOAD` of extensions, and queries against S3 views whose
credentials are already configured. Treat a panel's JavaScript as code you are choosing to
run, exactly as you would a shell script.

What a panel does **not** get is anything else the process could do:
...
- **No S3 credentials.** The S3 browser's keys live in the app's own Rust state and are
  used by DuckLocal's signing client; they are never written into the DuckDB connection, and
  the host module cannot reach them. A panel can still run SQL that uses httpfs if the user's
...
- **A panel in a tab has no window-level overlay.** The shell's dialogs, sheets, toasts and
  ...
  a tab the app's root holds that place — it has to, or the app's own dialogs would break — so
  ...
  attached by a hover listener, which has nothing to throw into, so a tooltip in a panel
  simply never appears. Build panel UI that stays inside its own bounds — an expanded region
  rather than a modal.
- **Reload is implemented by DuckLocal, not by the shell's watcher.** gpui-shell's hot reload
  ...
  reload watches `.js`/`.mjs` files only. Any other file a panel reads is not watched.
- **A reload compiles on the UI thread.** The script runtime is not sendable, so mounting a
  panel happens on the main thread; a very large module graph will make the window stutter
  while it loads. The first load is deferred until after the tab's first frame, so the tab
  appears immediately either way.
- **One connection for all panels, and no cancellation.** Panels do not block the main
  window, but they do block each other: they share one connection behind one lock, so a long
  query in one panel delays every other panel's. One connection each is not available — a
  host function is handed arguments, not a caller, so nothing at the moment `query()` runs
  says which panel asked. Cancelling a query in flight is not exposed either; keep a panel's
