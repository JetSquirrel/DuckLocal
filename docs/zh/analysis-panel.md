...
`ducklocal dash export --html <目录>` 把面板跑一次，并把它向数据库询问的内容写进一个自包含 HTML 文件，用来把面板里的数字发给没有 DuckLocal 的人。命令、选项和退出码见 [CLI 指南](cli.md#把面板导出为独立-html)。

报告给的是面板的数据，不是它的界面：它包含面板加载期间 `query()` 发出的语句、每条语句的结果表格，以及在结果是「每行一个名称 + 一个数值」时的一张柱状图；`catalog()` 调用则成为一节表与列的清单。它不包含面板的布局和图表——面板画的是原生组件，内容在布局时才确定，没有任何可读出并复现图表柱子的描述——所以这个文件并不假装自己就是面板。
